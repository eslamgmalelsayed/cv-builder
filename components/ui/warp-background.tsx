...
const Beam = ({
  ...
  const hue = useMemo(() => {
    if (!mounted) return 200; // Default value for SSR
    return Math.floor(seededRandom(index * 123) * 360);
  }, [index, mounted]);

  const ar = useMemo(() => {
    if (!mounted) return 6; // Default value for SSR
    return Math.floor(seededRandom(index * 456) * 10) + 1;
  }, [index, mounted]);
        ...
        style={
          {
            ...
            "--width": `${width}`,
            "--aspect-ratio": "6",
            "--background": `linear-gradient(hsl(200 80% 60%), transparent)`,
          } as React.CSSProperties
        }
        className={`absolute left-[var(--x)] top-0 [aspect-ratio:1/var(--aspect-ratio)] [background:var(--background)] [width:var(--width)] opacity-0`}
      />
    );
  }
