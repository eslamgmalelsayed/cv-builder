...
import { analyzeCV as analyzeCVClient, getEnhancedSuggestions } from "@/lib/ai-client";
...
const translations = {
  en: {
    ...
    noApiKey: "Note: Using basic analysis. Set NEXT_PUBLIC_GROQ_API_KEY for AI-powered suggestions.",
  },
  ar: {
    ...
    noApiKey: "ملاحظة: يتم استخدام التحليل الأساسي. قم بتعيين NEXT_PUBLIC_GROQ_API_KEY للحصول على اقتراحات مدعومة بالذكاء الاصطناعي.",
  },
};

export function EnhancedAIAssistantStatic({
  cvData,
  language = "en",
  onApplySuggestion,
  ...
  const analyzeCVHandler = async () => {
    if (!cvData || isAnalyzing) return;

    setIsAnalyzing(true);
    try {
      // Get ATS analysis using client-side AI
      const atsResult = await analyzeCVClient(cvData, language);
      setAtsAnalysis(atsResult);

      // Get enhanced suggestions using client-side AI
      const suggestionsResult = await getEnhancedSuggestions(cvData, language);
